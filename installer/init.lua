...
local installerVersion = "Jinto Setuper"
...
local function online()
    ...
    local filelist = split(assert(getInternetFile("https://raw.githubusercontent.com/matvey-mayner/Jinto/main/installer/list.txt")), "\n")
    for i, v in ipairs(filelist) do
        if v ~= "" then
            local url, name = table.unpack(split(v, ";"))
            table.insert(dists, {name = name, call = function(proxy)
                download("https://raw.githubusercontent.com/matvey-mayner/Jinto/main", proxy)
                download(url, proxy)
            end})
        end
    end

    table.insert(dists, {name = "Install Jinto", call = function(proxy)
        download("https://raw.githubusercontent.com/matvey-mayner/Jinto/main", proxy)
    end})
